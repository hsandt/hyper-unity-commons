/*=================== Replace with Prefab ===================
Unity Forum Community Thread https://discussions.unity.com/t/392983
...
namespace Community
{
    ...
    public class ReplaceWithPrefab : EditorWindow
    {
        ...
        Vector2                                             windowMinSize = new(450, 300);
        Vector2                                             windowMaxSize = new(800, 1000);
        ...
        {
            var window = (ReplaceWithPrefab) GetWindow(typeof(ReplaceWithPrefab));
            window.Show();
...
#region Draw Prefab and List
                ...
                if (prefab != null)
                {
                    if (objectsToReplace.Count > 0)
                    {
                        ...
                        scrollPosition = GUILayout.BeginScrollView(scrollPosition, EditorStyles.helpBox);
                        {
                            GUILayout.BeginHorizontal(EditorStyles.helpBox);
                            {
                                ...
                                {
                                    if (replacementPreferences.orderHierarchyToPreview && !replacementPreferences.renameObjects)
                                    {
                            ...
                            GUILayout.EndHorizontal();
                            foreach (var go in objectPreview)
                            {
                                GUILayout.BeginHorizontal(EditorStyles.toolbar);
                                GUILayout.Label(go);
                                GUILayout.EndHorizontal();
                    ...
                    {
                        if (!SearchWithTag && !SearchWithLayer)
                        {
...
#region Draw Bottom Buttons
                ...
                GUILayout.BeginHorizontal();
                {
                    if (prefab != null && objectsToReplace.Count > 0)
                    {
                        if (GUILayout.Button("Apply"))
                        {
                            ...
                            editMode = false;
                            EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene()); // Important so that we don't forget to save!
                        }
                        else
                        {
        ...
        void RenamePreview()
        {
            ...
            //Apply new names
            for (var n = 0; n < objectsToReplace.Count; n++)
            {
                if (replacementPreferences.renameObjects)
                {
                    count = GetCount(count, ExistingNumbers);
        ...
        void CopyContentsToNew(GameObject oldObject, GameObject newObject)
        {
            ...
            var components = oldObject.GetComponents(typeof(Component));
            foreach (var component in components)
            {
                ...
                while (copier == null)
                {
                    ...
                    {
                        if (componentType.BaseType == typeof(object) || componentType.BaseType == null)
                        {
            ...
            var childCount = oldObject.transform.childCount;
            for (var i = 0; i < oldObject.transform.childCount; i++)
            {
                var        child = oldObject.transform.GetChild(i).gameObject;
                GameObject newChild;
                var        newChildTransform = newObject.transform.Find(child.name);
        ...
        void GetSelection()
        {
            ...
            if (editMode && Selection.activeGameObject != null && (prefab == null || (!SearchWithTag && !SearchWithLayer)))
            {
                ...
                {
                    var t = PrefabUtility.GetPrefabAssetType(Selection.activeGameObject);
        ...
        public class DefaultComponentCopier : IComponentCopier
        {
            public void CopyComponent(ReplacementPreferences replacementPreferences, Component original, GameObject newObject)
            {
                ...
                var partAvoiders = componentPartAvoiders.ContainsKey(type) ? componentPartAvoiders[type] : null;
                ...
                {
                    if (!prop.CanWrite || prop.Name == "name" || prop.Name == "parent" || prop.MemberType == MemberTypes.Property)
                    {
                        continue;
                    }
        ...
        public class MeshRendererComponentCopier : IComponentCopier
        {
            public void CopyComponent(ReplacementPreferences replacementPreferences, Component original, GameObject newObject)
            {
                var meshRenderer    = (MeshRenderer) original;
                ...
                if (newMeshRenderer)
                {
                    ...
                    {
                        var CacheMaterials = new Material[meshRenderer.sharedMaterials.Length];
                        for (var a = 0; a < meshRenderer.sharedMaterials.Length; a++)
                        {
                            CacheMaterials[a] = meshRenderer.sharedMaterials[a];
                        }
                        for (var b = 0; b < CacheMaterials.Length; b++)
                        {
        ...
        public class SkinnedMeshRendererComponentCopier : IComponentCopier
        {
            public void CopyComponent(ReplacementPreferences replacementPreferences, Component original, GameObject newObject)
            {
                var meshRenderer    = (SkinnedMeshRenderer) original;
                var newMeshRenderer = newObject.GetComponent<SkinnedMeshRenderer>();
                // QUESTION Should we instantiate one if one is not present?
                if (newMeshRenderer)
                {
                    if (meshRenderer.sharedMaterials.Length == newMeshRenderer.sharedMaterials.Length)
