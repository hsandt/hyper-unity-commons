...
// required to have a copy of SetPropertyUtility
// for final build, just keep the SetPropertyUtility.cs script to avoid useless namespace cluttering and heavy build dependencies
using UnityEngine.UI.Extensions;

namespace UnityEngine.UI.Plus
{
    /// <summary>
    /// ImagePlus is an improvement over Unity native UI Image.
    /// This script needs to be updated each time Unity UI is updated.
    /// WIP: add preserve aspect ratio for Sliced images
    /// </summary>

    [AddComponentMenu("UI/Plus/ImagePlus", 11)]
    public class ImagePlus : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        public enum Type
        {
            Simple,
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
            ...
            switch (type)
            {
                case Type.Simple:
                    ...
                    break;
                case Type.Sliced:
                    GenerateSlicedSprite(toFill, m_PreserveAspect);
                    break;
                case Type.Tiled:
        ...
        // MODIFIED: added 2nd parameter
        private void GenerateSlicedSprite(VertexHelper toFill, bool lPreserveAspect)
        {
            if (!hasBorder)
            {
                // MODIFIED
                // GenerateSimpleSprite(toFill, false);
                GenerateSimpleSprite(toFill, lPreserveAspect);
                return;
            }
