...
public class SlicedCurveAnimator
{
    ...
    /// UB unless current phase is Intro or Loop
    public void PlayOutro()
    {
        #if UNITY_EDITOR
        Debug.AssertFormat(m_CurrentPhase is SlicedCurvePhase.Intro or SlicedCurvePhase.Loop,
            "[SlicedCurveAnimator] PlayOutro: phase is {0}, expected Intro or Loop", m_CurrentPhase);
        #endif
    ...
    private void EnterPhase(SlicedCurvePhase phase)
    {
        ...
        // by subtracting the last phase's duration; since Loop phase doesn't track time, and
