...
namespace HyperUnityCommons
{
	...
	public static class Physics2DUtil {
		...
		[Conditional("DEBUG")]
		public static void DrawRaycastMulti (Vector2 origin, Vector2 direction, float distance, IList<RaycastHit2D> hits, int nbResults, Color? color = null, float duration = 0f, float z = 0f)
		{
			// we can't draw an infinite ray so limit the draw distance
			if (float.IsPositiveInfinity(distance))
				distance = maxDrawLineDistance;
		...
		/// [Unity 5.6 new non-alloc Array overload] Raycast all colliders from origin toward direction over distance and store the result in hits. Draw with optional custom color for duration seconds at depth z.
		...
		/// [Unity 2019.1 new non-alloc List overload] Raycast all colliders from origin toward direction over distance and store the result in hits. Draw with optional custom color for duration seconds at depth z.
		public static int RaycastDebug (Vector2 origin, Vector2 direction, ContactFilter2D contactFilter, List<RaycastHit2D> results, float distance = Mathf.Infinity, Color? color = null, float duration = 0f, float z = 0f, string text = null)
		{
			int nbResults = Physics2D.Raycast(origin, direction, contactFilter, results, distance);
			DrawRaycastMulti(origin, direction, distance, results, nbResults, color, duration, z);
			#if UNITY_EDITOR
			if (!string.IsNullOrEmpty(text))
				DrawRaycastText(text, origin, direction, distance, color, duration);
			#endif
		...
		[Conditional("DEBUG")]
		public static void DrawBoxCastMulti (Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance, IList<RaycastHit2D> hits, int nbResults, Color? color = null, float duration = 0f, float z = 0f)
		{
			// we can't draw an infinite boxcast so limit the draw distance
			if (float.IsPositiveInfinity(distance))
				distance = maxDrawLineDistance;
		...
		/// [Unity 5.6 new non-alloc Array overload] Raycast all colliders from origin toward direction over distance and store the result in hits. Draw with optional custom color for duration seconds at depth z.
		public static int BoxCastDebug(Vector2 origin, Vector2 size, float angle, Vector2 direction, ContactFilter2D contactFilter, RaycastHit2D[] results, float distance = Mathf.Infinity, Color? color = null, float duration = 0f, float z = 0f)
		{
			int nbResults = Physics2D.BoxCast(origin, size, angle, direction, contactFilter, results, distance);
			DrawBoxCastMulti(origin, size, angle, direction, distance, results, nbResults, color, duration, z);
			return nbResults;
		}

		/// [Unity 2019.1 new non-alloc List overload] Raycast all colliders from origin toward direction over distance and store the result in results. Draw with optional custom color for duration seconds at depth z.
		public static int BoxCastDebug(Vector2 origin, Vector2 size, float angle, Vector2 direction, ContactFilter2D contactFilter, List<RaycastHit2D> results, float distance = Mathf.Infinity, Color? color = null, float duration = 0f, float z = 0f)
		{
			int nbResults = Physics2D.BoxCast(origin, size, angle, direction, contactFilter, results, distance);
			DrawBoxCastMulti(origin, size, angle, direction, distance, results, nbResults, color, duration, z);
			return nbResults;
		...
		/// [Unity 5.6 new non-alloc Array overload] Check overlapping colliders and draw area bounds for debug
		public static int OverlapAreaDebug(Vector2 pointA, Vector2 pointB, ContactFilter2D contactFilter, Collider2D[] results, int layerMask = Physics2D.DefaultRaycastLayers, float minDepth = -Mathf.Infinity, float maxDepth = Mathf.Infinity, Color? color = null, float duration = 0f)
		{
		#if UNITY_EDITOR
			Bounds debugBounds = new Bounds();
			debugBounds.min = (Vector3) pointA;
		...
		/// [Unity 2019.1 new non-alloc List overload] Check overlapping colliders and draw area bounds for debug
		public static int OverlapAreaDebug(Vector2 pointA, Vector2 pointB, ContactFilter2D contactFilter, List<Collider2D> results, int layerMask = Physics2D.DefaultRaycastLayers, float minDepth = -Mathf.Infinity, float maxDepth = Mathf.Infinity, Color? color = null, float duration = 0f)
		{
		#if UNITY_EDITOR
			Bounds debugBounds = new Bounds();
			debugBounds.min = (Vector3) pointA;
