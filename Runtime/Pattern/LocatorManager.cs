	...
	public class LocatorManager : SingletonManager<LocatorManager>
	{
		...
		public GameObject FindWithTag(string searchedTag)
		{
			...
			if (taggedGameObjects.TryGetValue(searchedTag, out go))
			{
				...
				taggedGameObjects.Remove(searchedTag);
				Debug.LogWarningFormat("Game object with tag {0} was memoized but got destroyed in the meantime. " +
				                       ...
				                       "(DontDestroyOnLoad) in particular.", searchedTag);
			}

			// search object with tag
			go = GameObject.FindWithTag(searchedTag);
			if (go != null)
			{
				// memoize game object and return it
				taggedGameObjects[searchedTag] = go;
				return go;
			}

			Debug.LogWarningFormat("Could not locate game object with tag {0}.", searchedTag);
			return null;
		}
	}
}
